...
uploadBtn.addEventListener('click', () => {
  ...
  // Map to student subject automatically
  const subject = teacherSchedule[sessionIdx].subject.split('-')[0].trim(); // extract main subject
  if (!notesData[subject]) notesData[subject] = [];
  notesData[subject].push({ name, url });

  pdfNameInput.value = '';
  pdfFileInput.value = '';

  alert(`PDF uploaded! It will appear in the ${subject} section for students.`);
});
